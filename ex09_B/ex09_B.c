...
int main() {
    ...
    printf("================================================================\n");
    printf("           ASSIGNMENT B - IMPROVED ALGORITHMS\n");
    printf("    Shell (Ciura) / Quick (Dual-Pivot) / Tree (AVL)\n");
        ...
        for (int asc = 0; asc < 2; asc++) {
            ...
            if (!dup) {
                ...
                printf("%-20s %18lld %16lld\n", "Tree Sort (AVL)", comp,
                    (long long)(sizeof(AVLNode) * count));
            }
            else {
    ...
    printf("================================================================\n");
    printf("Improvements:\n");
    printf("  Shell: Ciura gap sequence\n");
    printf("  Quick: Dual-pivot + insertion sort cutoff\n");
    printf("  Tree:  AVL (self-balancing)\n");
    printf("================================================================\n");
    system("pause");
    return 0;
}
...
// ==================== SHELL SORT (Ciura Gap) ====================
void shell_sort_improved(Student* arr, int n, Criteria c, int asc) {
    int gaps[] = { 701, 301, 132, 57, 23, 10, 4, 1 };
    int ng = 8, start = 0;
    while (start < ng && gaps[start] >= n) start++;

    for (int g = start; g < ng; g++) {
        int gap = gaps[g];
        for (int i = gap; i < n; i++) {
            Student temp = arr[i];
            int j;
...
void insertion_sort_range(Student* arr, int left, int right, Criteria c, int asc) {
    for (int i = left + 1; i <= right; i++) {
        Student key = arr[i];
        int j = i - 1;
        while (j >= left && compare_students(arr[j], key, c, asc) > 0) {
            arr[j + 1] = arr[j];
            j--;
        }
        arr[j + 1] = key;
...
void quick_sort_improved(Student* arr, int low, int high, Criteria c, int asc) {
    if (high - low < 10) {
        insertion_sort_range(arr, low, high, c, asc);
        return;
    }

    if (low < high) {
        int third = (high - low) / 3;
        int p1 = low + third, p2 = high - third;

        if (compare_students(arr[p1], arr[p2], c, asc) > 0) swap(&arr[p1], &arr[p2]);

        Student pivot1 = arr[p1], pivot2 = arr[p2];
        swap(&arr[p1], &arr[low]);
        swap(&arr[p2], &arr[high]);

        int i = low + 1, lt = low + 1, gt = high - 1;

        while (i <= gt) {
            if (compare_students(arr[i], pivot1, c, asc) < 0) {
                swap(&arr[i++], &arr[lt++]);
            }
            else if (compare_students(arr[i], pivot2, c, asc) > 0) {
                swap(&arr[i], &arr[gt--]);
            }
            else {
                i++;
        ...
        quick_sort_improved(arr, low, lt - 1, c, asc);
        quick_sort_improved(arr, lt + 1, gt - 1, c, asc);
        quick_sort_improved(arr, gt + 1, high, c, asc);
    }
}
...
AVLNode* insert_avl(AVLNode* node, Student data, Criteria c, int asc) {
    ...
    else node->right = insert_avl(node->right, data, c, asc);

    node->height = 1 + max_int(get_height(node->left), get_height(node->right));
    int bal = get_balance(node);

    if (bal > 1 && compare_students(data, node->left->data, c, asc) < 0)
        return rotate_right(node);
    if (bal < -1 && compare_students(data, node->right->data, c, asc) > 0)
...
void tree_sort_improved(Student* arr, int n, Criteria c, int asc) {
    AVLNode* root = NULL;
    for (int i = 0; i < n; i++) root = insert_avl(root, arr[i], c, asc);
    int idx = 0;
